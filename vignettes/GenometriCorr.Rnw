...
A user can can switch specific permutation types on by seting some of the {\tt mean.distance.permut.number},{\tt jaccard.measure.permut.number} and {\tt ecdf.area.permut.number} parameters to positive values along with {\tt permut.number=0}.
